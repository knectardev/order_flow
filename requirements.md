...
- **Session annotations (no persistence):** The **✎** control to the **right** of the HTF bias **`i`** legend (`orderflow_dashboard.html`) toggles a toolbar (**Select**, **Pen**, **Text**, **Box** translucent rectangle, **`input[type=color]`**, delete). **`src/ui/chartDrawOverlay.js`** stores freehand paths, text labels, and axis-aligned rectangles **only in memory** for the browser session. **Pen** captures pointer-drawn polylines; **Text** uses **`prompt()`** then anchors **`IBM Plex Mono`** labels at the click. **Box** drag-defines an axis-aligned rectangle (translucent fill tinted from the chosen color, solid outline in that color); after creation, **Select** can move the rectangle or resize it by dragging corners within ~10px of a vertex hit target. **Select** picks the top-most object (polyline proximity ~10px; text bounding box; box interior); drag translates paths, text, and boxes; corner drag resizes boxes; **⌫** / **Delete** / **Backspace** removes selection; **Escape** exits annotation mode. Color changes apply to the selection or the next stroke/text/box. Wheel over the overlay invokes **`handlePriceChartWheelZoom`** (`pan.js`) so candle zoom still runs; horizontal pan drag stays on **`#priceChart`** — turn annotations **off** for unobstructed chart interaction.
