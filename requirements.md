...
- When PHAT mode is active, an **`i`** legend control beside the Candle selector opens the same hover/focus popover pattern as the HTF bias ribbon key; it summarizes **neutral** (side-by-side translucent green/red swatches that match chart neutral-body alpha and indicate no P/b split, including the invisible **disagreement** case explained in copy) vs **P-shape** vs **b-shape** (larger candle-shaped swatches with direction-tinted green/red borders), **wick-tip rejection** semantics (**filled** circle = absorption, **open/hollow** circle = exhaustion unless wick liquidity crosses `phatExhaustionRingLiquidityThreshold`, then exhaustion renders filled like the chart), and an explicit **circle diameter** row (larger ring = stronger rejection, max one per bar) rendered with miniature wick stems, plus footer notes on width tier and wick stroke scaling.
...
- The lower **profile legend** acts as a visibility toggle for chart overlays: clicking **POC**, **VAH / VAL**, or **VWAP** toggles the associated lines/labels on the price chart and profile sidebar without changing bar data, profile fetch behavior, or replay state.
