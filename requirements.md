...
- The lower **profile legend** row describes candles/volume in plain language (**OHLC for the selected timeframe** — `1m` / `5m` / `15m` / `1h`; volume **per candle**) and acts as a visibility toggle for chart overlays: clicking **POC**, **VAH / VAL**, **VWAP**, or **OR 9:45** toggles the associated lines/labels on the price chart (and POC/VA/VWAP toggles still affect the profile sidebar where applicable) without changing bar data, profile fetch behavior, or replay state. **OR 9:45** draws dashed horizontal **high** and **low** for each Eastern **session calendar day** visible in the viewport: aggregated from **loaded** bars at the **active chart timeframe** whose bar-open bin intersects **[9:30, 9:45) America/New_York** (NYSE cash opening window; reference overlay, not the Python ORB engine’s first-bar rule). Lines span only that day’s candles in view; levels are read from the full **API replay** `allBars` when panned so morning context remains available for afternoon panes.
...
- Backtest execution is **DB-fire** when **`use_regime_filter=true`** and scope uses those watches **unless** **`watch_ids` ⊆ `{orb}`** — **`orb`** is derived from **`bars`** at runtime (**`signalSource`/`metadata_json.fire_source`: `derived_runtime`**, **`timeframe` must be `5m`**); **`fires`** rows are not required for **`orb`**. Otherwise scoped **`fires`** absent in-window fails fast with a clear error.
...
- Runtime **`orb`** fires set **`price`** to the breakout bar’s **high** (long) or **low** (short), not that bar’s **close** (`opening_range_breakout.emit`). With **`entry_next_bar_open`** false, the simulator opens on the signal bar using that price. **ORB-only:** mechanical **`try_intrabar_exit`** (stop-loss / take-profit honors) is **skipped on the entry bar** (`watch_id == orb` and `entry_index == current bar index`) so a fill at the breakout extreme does not instantly re-test SL/TP against the same bar’s full `high`/`low` (which was unrealistically stopping tight templates). Intrabar SL/TP resumes from the **next** bar; **flip** and other fire-driven exits on the entry bar are unchanged. Canonical watches still evaluate both intrabar passes on every bar including the signal bar, so tight stops there can still same-bar exit.
...
- Dashboard `Performance` panel includes:
  ...
  - Price chart overlays backtest executions for the visible window:
    ...
    - when both entry and exit map to **visible** chart bars, a **dashed** segment links the two marker anchor points (connector drawn **before** the E/X glyphs on the same pass). The overlay is painted **last** in the price canvas pass — **after** VWAP, fire halos, swing triangles, CVD–price divergence segments, session-axis chrome, FIRE-selection verticals, hover crosshair, and the NOW line — so nothing overdraws it; the connector uses a dark under-stroke plus a bright dashed top stroke (`[6,5]`). **Pipeline swing** markers are separate: small filled ▲/▼ at bar highs/lows (red / teal) with **no** E or X label — they never draw entry–exit connectors. After **Run Backtest** completes, `drawPriceChart()` is invoked from the run handler so the main chart immediately reflects new `compare.*.trades` (the Performance panel alone does not trigger a price repaint). Bar resolution prefers an exact `bar_time` match to `entryTime` / `exitTime`; if that fails, the chart snaps to the nearest visible bar within **half** the active timeframe bin so coarse charts still pair markers with persisted trades.
