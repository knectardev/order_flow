...
- **Session annotations (no persistence):** The **✎** control to the **right** of the HTF bias **`i`** legend (`orderflow_dashboard.html`) toggles a toolbar (**Select**, **Pen**, **Text**, **Box**, **L** long placement, **S** short placement, **`input[type=color]`**, delete). **`src/ui/chartDrawOverlay.js`** stores freehand paths, text labels, axis-aligned rectangles, and long/short overlays **only in memory** for the browser session. **Pen** captures pointer-drawn polylines; **Text** uses **`prompt()`** then **`IBM Plex Mono`** labels at the click. **Box**: drag-axis rectangle (translucent fill from **`input`** color); **Select** moves or corner-resizes (~10px). **L** and **S**: same drag gesture; entry is **mid-height** of the drag box; each overlay stores **`side: 'long' | 'short'`**. Stored spans are **`hProfit`** (reward, **always green**) and **`hLoss`** (risk, **always red**): **long** renders green above / red below; **short** renders red above / green below. **`input`** tints horizontal **entry** stroke + risk-ratio pill rim only (not fills). Pill: **Risk Ratio** = **`hProfit ÷ hLoss`** (~0 denominator → **—**); pill sits centered in **green**. **Select**: four handles (~10px) — **TL** / **BL** reshape stacked bands from corners (mirrored implementation for short vs long), **MR** width, **ML** + interior drag translates. **⌫** / **Delete** / **Backspace** clears selection; **Escape** exits mode. Overlay wheel uses **`handlePriceChartWheelZoom`** (`pan.js`); toggle annotations off to drag-pan **`#priceChart`** cleanly.
