...
- Pipeline: `pipeline/src/orderflow_pipeline/*` computes bars, events, fires, regime **integer ranks** (`v_rank`, `d_rank`, 3-bar smoothed rounded 1–5 from an **endpoint** trailing-window percentile (`(n−1)` stretch, window midpoint → bucket 3)), **continuous scatter coordinates** **`vol_score`** / **`depth_score`** (same window computed in **one rolling pass**: mid-rank / average-rank percentile → values strictly in **(1, 5)** when the window has more than one valid sample — avoids artificial mass on **exactly 1.0 / 5.0**), session VWAP, directional bias states, and DB writes. **Implementer constraints (imports, ETL, no legacy scatter):** §4.3. **Contract change:** after updating this classifier, run a **full** **`python -m orderflow_pipeline.cli rebuild`** (same as other persisted-bar semantics changes) so DuckDB / JSON shards do not mix pre-change and post-change `vol_score`/`depth_score`. Downstream code does not treat **`volScore === 3`** as “window median”; only the **integer** rank path preserves the midpoint→3 mapping.
  ...
  - `bars` rows additionally carry **session CVD** (`session_cvd` BIGINT — cumulative session delta at each bar), **aggressor size stats** (`aggressive_buy_count`, `aggressive_sell_count`, `avg_aggressive_buy_size`, `avg_aggressive_sell_size`, `size_imbalance_ratio`), `vwap`, PHAT features (`top_cvd`, `bottom_cvd`, `top_cvd_norm`, `bottom_cvd_norm`, `cvd_imbalance`, `top_body_volume_ratio`, `bottom_body_volume_ratio`, `upper_wick_liquidity`, `lower_wick_liquidity`, `upper_wick_ticks`, `lower_wick_ticks`, `high_before_low`, `rejection_side`, `rejection_strength`, `rejection_type`), bias columns (`bias_state`, `parent_1h_bias`, `parent_15m_bias`), and **velocity matrix** columns: `session_kind` (`rth`|`globex`, matching ingest filter), `path_length_ticks` / `vw_path_length` (trade-to-trade path in integer ticks and volume-weighted sum of \(|\Delta\text{ticks}|\times\text{size}\) per edge — same pass as OHLC), `displacement_ticks` / `abs_displacement_ticks`, `pld_ratio` (**path_length_ticks / bar range in ticks** — range from `high`/`low` on the same integer tick grid as path; **null** when printed range is **0**; **not** capped), `flip_count` (**within-bar only** — crosses from the prior bar’s last trade into this bar do **not** increment flips), `flip_rate` (**JSON null** when `trade_count ≤ 1`, else `flip_count / (trade_count − 1)`), `jitter_regime` / `conviction_regime` (causal terciles from trailing prior bars with the **same** `session_kind` only — no RTH/globex mixing; **≥ 30** prior same-kind bars required before labels are non-null; conviction mapping is **inverted** vs flip percentile so low flip ⇒ High conviction), and **`trade_context`** (`favorable` \| `avoid` \| `watch` \| `neutral`) computed **after** regimes are stamped on every aggregate/rebuild: Low jitter + High conviction ⇒ `favorable`; High + Low ⇒ `avoid`; High + High ⇒ `watch`; else **`neutral`**. **Warmup / NULL regimes** fall through to **`neutral`** with no separate DB token — `neutral` conflates true mid/mid with insufficient history (no composite dot in either case). Legacy databases with NULL `trade_context` can be backfilled with **`python scripts/migrate_trade_context.py`** passing the DuckDB file path as the sole argument (same CASE semantics as ingest); fresh ingests do **not** require this script. Path segments **do not** use a price lag from before the **session open** (first trade of the session starts tape geometry); segments **do** include trade-to-trade continuity **across bar boundaries** within the session. Tables **`swing_events`** (fractal swings on price + session CVD, with `swing_lookback` on every row) and **`divergence_events`** (CVD–price divergences with ingest thresholds and `size_confirmation`) are written in the same session transaction as bars when using DB ingest.
...
- `/bars`, `/events`, `/fires` payloads include `vwap`, `barEndTime` (exclusive bar end, ISO-Z), `sessionCvd` (session cumulative delta), aggressor fields (`aggressiveBuyCount`, `aggressiveSellCount`, `avgAggressiveBuySize`, `avgAggressiveSellSize`, `sizeImbalanceRatio`), continuous regime coordinates `volScore` / `depthScore` (open-interval **(1, 5)** after warmup for typical multi-bar windows — JSON null in warmup / zero-volume like `vRank`/`dRank`), PHAT fields (`topCvd`, `bottomCvd`, `topCvdNorm`, `bottomCvdNorm`, `cvdImbalance`, `topBodyVolumeRatio`, `bottomBodyVolumeRatio`, `upperWickLiquidity`, `lowerWickLiquidity`, `upperWickTicks`, `lowerWickTicks`, `highBeforeLow`, `rejectionSide`, `rejectionStrength`, `rejectionType`), velocity fields (`pldRatio`, `flipRate`, `vwPathLength`, `pathLengthTicks`, `jitterRegime`, `convictionRegime`, `tradeContext` — regimes null when not computed or insufficient history; `tradeContext` is always one of the four tokens after ingest), and bias fields (`biasState`, `biasH1`, `bias15m`) projected from persisted columns via `_attach_htf_bias`.
- Storage: DuckDB (`data/orderflow.duckdb` by default). **`data/**/*.bak`** copies and **`data/rebuild_log.txt`** are **gitignored** local artifacts (often large). **Ingest memory (DuckDB):** very large sessions can hit `OutOfMemory` while inserting `bar_volume_profile` — the writer **chunks** large profile inserts (default **250_000** rows per chunk; override with env **`ORDERFLOW_DUCKDB_PROFILE_CHUNK`**). Optional **`ORDERFLOW_DUCKDB_MEMORY_LIMIT`** (e.g. `8GB`) and **`ORDERFLOW_DUCKDB_TEMP_DIR`** (spill directory; use forward slashes on Windows) are read in **`db.connect`** to tune DuckDB.
...
- **Chart overlays:** `chartOverlayVisibility` toggles POC/VA/VWAP/OR 9:45 plus **velocity UX**: **`velocityRegimeLane`** (default **on**) draws a **17px** band (**8px** jitter row + **1px** sep + **8px** conviction row) between the price pane and the volume histogram using **muted** teal / gray / amber / violet lane fills (`src/config/constants.js` — **never** candle bull/bear greens/reds for regime encoding); **both lane rows** have **per-bar hit targets** in **`state.chartHits`** — **`regimeLaneJitter`** (top) and **`regimeLaneConviction`** (bottom); **`tooltip.js`** explains jitter via trailing **log(PLD)** terciles and conviction via **flip rate** terciles with **inverted** labels (matches **`velocity_regime.py`**), plus the same velocity metric lines / dev matrix overlay as bar tooltips when enabled. The active bar column redraws **brighter** row fills (`REGIME_JITTER_LANE_FILL_HOVER`, **`REGIME_CONVICTION_LANE_FILL_HOVER`**, neutral hover) and **faint outlines** on both bricks when **`selection.hoverBarTime`** matches (candle, either lane row, or matrix link). **`tradeContextDots`** (default **on**) draws filled composite dots (~**3.5px** radius) at **`yScale(high) − OVERLAY_STACK_STEP`** (**`OVERLAY_STACK_STEP = 6`**) with **`rgba(45,212,191,.95)`** / **`rgba(245,158,11,.95)`** / **`rgba(167,139,250,.95)`** for **`favorable`** / **`avoid`** / **`watch`**; **`neutral`** draws **no** dot. The prior legacy velocity tint/border overlays were removed from UI and rendering, leaving lane+dots as the only velocity visual channels. **`velocityMatrixDev`** (default **off**) appends the notes.txt **3×3** matrix cell index **1–9** (rows Low→High jitter, cols Low→High conviction) to **bar tooltips only** — no on-chart labels; the profile legend includes a **dev hint line** whose opacity tracks this toggle via **`data-overlay-hint-for`** + **`is-off`**. Bar hover tooltips also list **`pldRatio`**, **`flipRate`**, **`jitterRegime`**, **`convictionRegime`**, **`tradeContext`** when present. Legend buttons mirror **`data-overlay-toggle`** in `orderflow_dashboard.html` (`bindChartOverlayLegendToggles` + hint sync in `main.js`). **`drawPriceChart` paint order:** background → POC/VA ref lines → bias ribbon → session dividers → candles/wicks → **regime lane** → **volume** → **composite dots** → opening-range box → VWAP → fires/events/profile chrome (composite dots after volume so they stay above wicks in the price pane). **Follow-up (not yet shipped):** align primitive/event glyph Y with the tier-**2** stack rung (**`yHigh − 12`**) so composite dots (**tier 1**) and legacy glyphs never collide — until then occasional overlap is possible. Overlay visibility persists in **`localStorage`** **`orderflow_dashboard_chart_overlay_visibility`**.
- Velocity legend explainers support **hover** for quick reads and **click-to-pin** for persistent reading (`Esc` / outside click to dismiss). Trade-dot explainer rows use the same colored dot glyphs (teal/amber/violet) inline for faster visual parsing. When **`velocityMatrixDev`** is on, bar tooltips include both the numeric cell and a compact 3×3 mini-matrix with the active cell highlighted.
- **Jitter vs absolute PLD:** `jitter_regime` classifies **`pld_ratio`** **relative** to up to **200** prior bars with the same **`session_kind`**, using the **same** midrank percentile as ingest but computed on **`log(pld_ratio)`** for **positive** finite priors and current bar (NULL or non-positive `pld_ratio` ⇒ NULL jitter — see **`pipeline/src/orderflow_pipeline/velocity_regime.py`**). **`Low`** means the current bar lands in the **bottom third** of that trailing distribution. After any change to **`pld_ratio`** or jitter rules, run a **full** **`python -m orderflow_pipeline.cli rebuild`** so stored bars and regimes stay consistent. Spot-check any bar with **`python scripts/diagnose_pld_jitter_bar.py <duckdb_path> --timeframe <tf> --session-kind <rth|globex> --bar-time "<UTC TIMESTAMP>"`** (naive **`bar_time`** as stored). For tail sanity on rebuilt data, e.g. **`SELECT approx_quantile(pld_ratio, 0.99) FROM bars WHERE timeframe = '5m' AND session_kind = 'rth'`** — expect a mass **well below** the legacy **100** cap spike on old displacement-based series; path/range ratios are instrument-dependent (often moderate upper quantiles vs capped tails). Optional follow-up: re-run any **regime-filter ON vs OFF** backtest comparison (e.g. ORB-style scopes) against your pre-rebuild baseline to see whether velocity-regime distribution changes moved performance.
- **Viewport state:** `chartViewEnd` for panned history vs live edge; **`chartVisibleBars`** (wheel-driven horizontal zoom — how many candles are mapped across the chart width, clamped per loaded/cursor bounds). **Simulated future (UI-only):** **`chartFutureBlankSlots`** reserves empty slots to the right of the last OHLC column — both **panned-behind-live** and **live / streaming** (`_getViewedBars()` sizes `viewportSlotCount` to **`chartVisibleBars`** while shortening `viewedBars`); price/regime/volume ink there is cleared to the chart background; **NOW** is **amber** on that boundary (otherwise teal centered on the last bar); x-axis labels extend into blanks via timeframe bin spacing; horizontal drag on **NOW** moves the divider with the pointer (`pan.js`, document-level move in `tooltip.js`). **Horizontal pan-drag** is allowed when **`replay.allBars.length`** exceeds clamped **`chartVisibleBars`** or **`chartViewEnd`** trails **`replay.cursor`** (`pan.js` **`_panAvailable`**); **`mousedown`** clears the competing gesture (**NOW** vs pan); **`mouseup`** / window **`blur`** / **`visibilitychange`** (**hidden**) clears **`isDraggingPlayhead`** / **`isPanningChart`** via **`_clearChartDragModes`** so drag state cannot stick after Pause / UI clicks. **`seek()` does not clear** **`chartFutureBlankSlots`** (Resume Stream keeps playhead offset). **`chartFutureBlankSlots` resets to 0** on **fresh timeline reload**, **`returnToLiveEdge`** (**↺ Live**), when horizontal zoom **clamps** it, and when a fire brush recenters **`chartViewEnd`** (`selection.js`). Coupling **`chartViewEnd`** to **`replay.cursor`** via pan (`_setViewEnd`, **`chartViewEnd`** → **`null`**) **does not** clear blanks. Delta/CVD layout uses the same slot count via **`_getViewedBars().viewportSlotCount`** (`flowChart.js`, `cvdChart.js`, `subchartHit.js`). **`matrix.js`** uses the same viewed-bar slice via **`_getViewedBars()`**. The first time the user pans off the live edge while `replay.allFires` is still empty, `precomputeAllFires()` runs (saving and restoring `chartViewEnd` so the viewport is not reset). **When `replay.cursor` already equals `replay.allBars.length` (tape end), `precomputeAllFires()` snapshots `replay.allFires` from `canonicalFires` with no `seek()` — no redundant full replay.** When `precomputeAllFires` is triggered from `_loadAllSessionsFromApi` after the initial `seekAsync(length)` (see below), `allFires` is filled that way without an extra `seek` pass. Chart halos for ★/◆/🛡/🎯 in **API replay** merge `allFires` and `canonicalFires` (when panned) and filter by **`activeCanonicalFireTypes`**. Matching uses `watchId|bar_time_ms`. In real mode, anchored VWAP is drawn from the same cumulative `allBars[0..cursor)` series for both live and panned views (visible slice is windowed from that), avoiding a false straight↔curved change at pan boundaries.
...
- **Backtest state:** `backtest` stores run params (`scope`, **`gateProfile`** (`none` \| `rank_only` \| `trade_context_only` \| `both`), **`legacyStrictThresholds`** → POST **`use_regime_filter`** for pipeline breakout/fade/etc. threshold strictness only, **`tradeContextAllowedStr`** → POST **`trade_context_allowed`**, optional **`compareFourModes`**, `initialCapital`, `commissionPerSide`, `slippageTicks`, `qty`, optional `stopLossTicks` / `takeProfitTicks`, optional **`regimeExitScaleEnabled`** / **`regimeExitScaleMode`** (`range_pct` \| `v_rank`) for regime-based template SL/TP scaling, **`entryNextBarOpen`** / **`entryGapGuardMaxTicks`** (next-bar integrity execution and optional gap guard), snapshots after Run Backtest, marker toggles), **`compareFour`** when a four-way compare finishes, `lastRunScope` (scope string stamped after each successful run for the metrics row while results are shown), latest `runId`, latest `stats`, and fetched `equity` / `trades` payloads plus loading/error UI flags. After API replay load, the dashboard does **not** query or render the latest persisted DuckDB run; metrics, equity, and chart overlays stay empty until the user selects a **Backtest scope** and clicks **Run Backtest**.
...
- **Session annotations (no persistence):** The **toolbar** (**Select**, **Pen**, **Text**, **Box**, **L**, **S**, **`input[type=color]`**, ⌫) next to **`✎`** in `orderflow_dashboard.html` stays **always visible**. The **`✎`** control toggles only the **drawing layer** — whether **`#chartDrawOverlay`** receives pointers (`chart-wrap-annotations-active` on **`#priceChartWrap`**); when off, the price chart keeps normal pan/hover. Picking **Pen**, **Text**, **Box**, **L**, or **S** turns the layer **on** if it was off. **`src/ui/chartDrawOverlay.js`** stores annotations **only in memory** for the browser session. **Pen** captures pointer-drawn polylines; **Text** uses **`prompt()`** then **`IBM Plex Mono`** labels at the click. **Box**: drag-axis rectangle (translucent fill from **`input`** color); **Select** moves or corner-resizes (~10px). **L** and **S**: same drag gesture; entry is **mid-height** of the drag box; each overlay stores **`side: 'long' | 'short'`**. Stored spans are **`hProfit`** (reward, **always green**) and **`hLoss`** (risk, **always red**): **long** renders green above / red below; **short** renders red above / green below. **`input`** tints horizontal **entry** stroke + risk-ratio pill rim only (not fills). Pill: **Risk Ratio** = **`hProfit ÷ hLoss`** (~0 denominator → **—**); pill sits centered in **green**. **Select**: four handles (~10px) — **TL** / **BL** reshape stacked bands from corners (mirrored implementation for short vs long), **MR** width, **ML** + interior drag translates. **⌫** / **Delete** / **Backspace** clears selection; **Escape** exits drawing layer (toolbar remains visible). Overlay wheel uses **`handlePriceChartWheelZoom`** (`pan.js`); turn **✎** off to drag-pan **`#priceChart`** without hitting the overlay.
...
- **Volume profile column:** when the right-hand profile bins draw (POC/VAH/VAL resolved), a **hairline** vertical rule at the **left edge of the bin rects** runs from the top of the candle pane through the bottom of the **volume sub-band** — muted gray (`priceChart.js`) — anchoring the column against the main chart gutter.
...
- The lower **profile legend** row describes candles/volume in plain language (**OHLC for the selected timeframe** — `1m` / `5m` / `15m` / `1h`; volume **per candle**) and acts as a visibility toggle for chart overlays: clicking **POC**, **VAH / VAL**, **VWAP**, or **OR 9:45** toggles the associated lines/labels on the price chart (and POC/VA/VWAP toggles still affect the profile sidebar where applicable) without changing bar data, profile fetch behavior, or replay state. **OR 9:45** draws a light-purple translucent box for each Eastern **session calendar day** visible in the viewport: vertical bounds are the opening-range **high/low** aggregated from **loaded** bars at the **active chart timeframe** whose bar-open bin intersects **[9:30, 9:45) America/New_York** (NYSE cash opening window), while horizontal bounds match that same **9:30–9:45 ET** formation window (aligned with runtime **`orb`** when **`5m`** bars define the range — not an extended morning strip). Overlay levels are read from the full **API replay** `allBars` when panned so morning context remains available for afternoon panes.
...
- The MVP backtester reuses persisted **`fires`** as strategy triggers for canonical watches (`breakout`, `fade`, `absorptionWall`, `valueEdgeReject`).
- Backtest execution is **DB-fire** when **`use_regime_filter=true`** and scope uses those watches **unless** **`watch_ids` ⊆ `{orb}`** — **`orb`** is derived from **`bars`** at runtime (**`signalSource`/`metadata_json.fire_source`: `derived_runtime`**, **`timeframe` must be `5m`**); **`fires`** rows are not required for **`orb`**. If runtime derivation yields **no** ORB fires (narrow window, regime gate, rules), the run still completes with **`tradeCount=0`** (empty signal stream). Otherwise scoped **`fires`** absent in-window fails fast with a clear error for non-runtime scopes.
- Dashboard **`POST /api/backtest/run`** **`from`/`to`**: after API replay load, bounds come from **`replay.dateRange`**. If that object is not ready yet, the client falls back to **`GET /date-range?timeframe=<backtest tf>`** on the same API base (instead of a rolling **last-24h wall-clock** slice that often misses historical DuckDB data).
...
- **Derived-fire path when DB fires are not used:** `POST` **`use_regime_filter=false`** still selects **`signalSource=derived_no_regime`** (legacy strategies relaxed per config). That flag does **not** imply **`rank_gate_enabled`** / **`trade_context_gate_enabled`** — entry gates are controlled separately on **`POST /api/backtest/run`**. **`rank_gate_enabled`** applies the ORB {(3,2),(3,3),(4,2),(4,3)} matrix check only to **`orb`** signals (and filters persisted **`fires`** at **`orb`** bar times before simulation). **`trade_context_gate_enabled`** filters every watch using **`bars.trade_context`** ∈ **`trade_context_allowed`** (default **`["favorable"]`** when the list is omitted and the gate is on).
- Strategy defaults include a **5m ORB watch template** in `config/strategy_defaults.json`: `watch_exit_ticks.orb.stop_loss_ticks=28`, `watch_exit_ticks.orb.take_profit_ticks=56`. After regime template scaling (when enabled), the engine applies an **`orb`** structural floor: effective stop ticks are at least the distance from fill to the **opposite side of the opening range** (long → below **`or_low`**, short → above **`or_high`**) plus a small tick buffer — unless the operator sets a run-wide **`stop_loss_ticks`** override. This prevents fixed-template stops from sitting **inside** the opening-range structure when entries confirm late in the extension.
...
- SL/TP resolution: optional **`BrokerConfig.stop_loss_ticks` / `take_profit_ticks`** act as **run-wide overrides** when either is non-null; otherwise each new position inherits **`LegacyFallbackConfig`** timeframe defaults merged with optional **`watch_exit_ticks`** per canonical watch (`pipeline/src/orderflow_pipeline/strategies/config.py`, resolver `strategies/exit_ticks.py`). When **`regime_exit_scale_enabled`** is **true** on the merged **`BrokerConfig`** and **neither** broker tick override is set, template ticks are scaled per trade by **`strategies/regime_exit_scale.apply_regime_exit_scale`** using **`range_pct`** / **`v_rank`** from the **entry bar** (including deferred **`entry_next_bar_open`** fills); scaling is skipped when broker overrides are present. Run **`metadata_json.exit_ticks_resolution`** is **`strategy_defaults_regime_scaled`** when scaling applies; persisted trades may include **`stop_loss_ticks_effective`**, **`take_profit_ticks_effective`**, **`slippage_to_stop_ratio`**; **`metadata_json.mean_slippage_to_stop_ratio`** summarizes slip vs effective SL where applicable.
...
- Runtime **`orb`** uses a hardcoded **break -> retest -> confirm** contract on `5m` bars: opening range is aggregated from bars whose interval intersects **[9:30, 9:45) America/New_York**; after that window, a side must first break the OR boundary, then a **later** bar must retest and close back through that same boundary to confirm. Long confirm requires `low <= or_high` and `close > or_high`; short confirm requires `high >= or_low` and `close < or_low`. With **`rank_gate_enabled=true`**, the ORB cell gate {(3,2),(3,3),(4,2),(4,3)} is evaluated on the **confirmation bar** (together with stored break-bar rank state); when **`rank_gate_enabled=false`**, that matrix gate is skipped. With **`trade_context_gate_enabled=true`**, the confirmation bar’s **`trade_context`** must be in **`trade_context_allowed`**. Pipeline **`derive_fires_from_bars`** defaults ORB rank gating to **`LegacyFallbackConfig.use_regime_filter`** when **`rank_gate_enabled`** is omitted (CLI parity); the HTTP API passes explicit gate flags. Runtime ORB emits **`price=confirmation_bar_close`** and locks to one direction per session after first confirmation. With **`entry_next_bar_open`** false, the simulator opens on the signal bar at that emitted price. **ORB-only:** mechanical **`try_intrabar_exit`** (stop-loss / take-profit honors) is **skipped on the entry bar** (`watch_id == orb` and `entry_index == current bar index`). Intrabar SL/TP resumes from the **next** bar; **flip** and other fire-driven exits on the entry bar are unchanged. Canonical watches still evaluate both intrabar passes on every bar including the signal bar, so tight stops there can still same-bar exit.
- ORB confirmation guardrails: pending break setups expire if not confirmed within **6 bars** (`retest_max_bars=6`), pending long (short) setup invalidates on meaningful breach of the opposite OR side, and reclaim confirmation uses a **1 tick** tolerance around the OR boundary (`reclaim_tolerance_ticks=1`). This curbs late-session confirmations that are visually disconnected from the original breakout probe while still allowing near-level reclaim closes.
- ORB freshness window: breakout search is limited to the first **18 bars** after OR finalization (`break_search_max_bars_after_or=18`, ~90 minutes on 5m), and confirmations after **11:45 ET** are rejected (`confirm_last_min_et=705`). This keeps entries tied to the opening-range thesis while avoiding very late-session confirms.
- ORB rank-gate flexibility: when **`rank_gate_enabled`** is ON, rank eligibility can be satisfied by either the **breakout bar** or the **confirmation bar**, reducing misses where the breakout context is strong but the retest bar lands in an adjacent/non-allowed cell.
- Backtest UI defaults for this path: **Scale template SL/TP by regime** is checked by default and **Regime mode** defaults to `range_pct`; with blank run-wide Stop/Take fields, ORB runs use the 5m ORB template above and apply regime scaling by default.
...
- Fire mapping:
  - `breakout`, **`orb`**: trade with fire direction.
...
- `backtest_trades` stores closed-trade records (`entry_time`, `exit_time`, direction, prices, gross/net P&L, bars held, optional `exit_reason`, source watch). Upgrade-safe columns **`stop_loss_ticks_effective`**, **`take_profit_ticks_effective`**, **`slippage_to_stop_ratio`** record regime-scaled exit distances and slip diagnostics when present; **`entry_trade_context`** stores the **`bars.trade_context`** value on the **entry fill bar** when present.
...
- `POST /api/backtest/run` broker fields (`initial_capital`, `qty`, `slippage_ticks`, `commission_per_side`, `tick_size`, `point_value`, `stop_loss_ticks`, `take_profit_ticks`, **`regime_exit_scale_enabled`**, **`regime_exit_scale_mode`**, **`regime_sl_mult_min`**, **`regime_sl_mult_max`**, **`regime_tp_mult_min`**, **`regime_tp_mult_max`**, **`regime_sl_floor_ticks`**, **`regime_v_rank_sl_mults`**, **`regime_v_rank_tp_mults`**) are **optional**: omitted keys use the broker defaults JSON merge; present keys override individually (explicit JSON `null` on the tick fields clears a file-provided value for that field). **`regime_exit_scale_mode`** must be **`range_pct`** or **`v_rank`**; SL/TP multiplier ranges must satisfy **`max >= min`**; **`regime_sl_floor_ticks`** must be **≥ 0** when set; **`regime_v_rank_*_mults`** must be **length-5** lists when provided.
...
- `GET /api/backtest/trades` trade objects include **`exitReason`** (`flip`, `stop_loss`, `take_profit`, `end_of_window`, or null for legacy rows). When persisted, responses include optional **`stopLossTicksEffective`**, **`takeProfitTicksEffective`**, **`slippageToStopRatio`**, **`entryTradeContext`** (camelCase JSON).
...
- Dashboard `Performance` panel includes:
  ...
  - **Entry gates:** dropdown **`none` / `rank_only` / `trade_context_only` / `both`** maps to **`rank_gate_enabled`** and **`trade_context_gate_enabled`**. **Legacy strict thresholds** toggles **`use_regime_filter`** (pipeline breakout/fade/etc. strictness only). **Allowed trade contexts** is a comma-separated list for **`trade_context_allowed`** when the trade-context gate is on.
  - Optional **Compare all four gate modes:** runs four parallel **`POST /api/backtest/run`** calls (all combinations of the two booleans), fetches equity for each **`runId`**, and draws **four** strategy curves on the equity canvas (distinct colors per mode: gray / orange / purple / teal in canonical order). Non-selected modes use **dashed** strokes; the dropdown-selected profile is **solid** and drawn on top. Overlapping identical equity paths still read as one trace—use the compare-four summary table (trade count / net P&L per mode) to confirm differences. Client-side only (**cannot combine** with null-hypothesis in one click).
  - After each run, a read-only **per trade_context** summary table (counts, win rate, average net, sum net) aggregates **`entryTradeContext`** from the primary run’s trades.
  ...
  - Optional **Null hypothesis (frequency-matched random)** (green label): when **exactly one watch** is selected in Backtest scope (and scope is **not** **`orb`**), the dashboard attaches **`null_hypothesis`** to the **Regime filter ON** POST only (compare OFF runs do **not** request NH). Checkbox state persists in **`runParams`**; the control is disabled for **All canonical watches** / unset scope / **`orb`** scope. While NH is requested, the Performance status line explains that parity search can take **many minutes** on wide windows (single synchronous **`POST /api/backtest/run`** — no progress meter). Legend visibility uses **`hidden`** on the NH legend row with CSS so **`display: inline-flex`** on legend items cannot leave the green dot visible when no NH payload exists; if NH is checked but **`POST /api/backtest/run`** returns no **`nullHypothesis`** object (e.g. stale API process), the browser console warns.
  - Metrics cards use a single primary column (teal). When a **Null hypothesis** companion run is present (`nullHypothesis.runId` in the POST response and hydrated client-side), each metric card shows a **second row** in **green** (`#2ecc71`) under the baseline row with NH **scope**, **run id** (8-char), **trade count**, **win rate**, **Sharpe**, **max drawdown**, **net P&L**, and **equity point count**. The buy-and-hold benchmark is pink when **Show buy & hold benchmark** is on; **Null hypothesis** equity (when returned) is drawn green.
  - Price chart overlays backtest executions for the visible window:
    - entry marker = prominent pointer badge with a bold text label (`L` for long, `S` for short, fallback `E`) anchored above the entry price
    - exit marker = prominent pointer badge with bold `X` text anchored below the exit price
    - labels use high-contrast fill, dark outline, and drop-shadow for readability over dense candles
    - when both entry and exit map to **visible** chart bars, a **dashed** segment links the two marker anchor points (connector drawn **before** the badges on the same pass). The overlay is painted **last** in the price canvas pass — **after** VWAP, fire halos, swing triangles, CVD–price divergence segments, session-axis chrome, FIRE-selection verticals, hover crosshair, and the NOW line — so nothing overdraws it; the connector uses a dark under-stroke plus a bright dashed top stroke (`[6,5]`). **Pipeline swing** markers are separate: small filled ▲/▼ at bar highs/lows (red / teal) with **no** trade badges — they never draw entry–exit connectors. After **Run Backtest** completes, `drawPriceChart()` is invoked from the run handler so the main chart immediately reflects new `compare.*.trades` (the Performance panel alone does not trigger a price repaint). Bar resolution prefers an exact `bar_time` match to `entryTime` / `exitTime`; if that fails, the chart snaps to the nearest visible bar within **half** the active timeframe bin so coarse charts still pair markers with persisted trades.
    - markers remain offset from candle bodies for visual separation
  ...
  - Backtest status line includes skipped-fire summary for ON always; adds OFF summary only when paired OFF run exists; appends **Null hypothesis** skipped summary or **`NH skipped (reason)`** when that companion path ran; when the last run requested regime scaling, appends **`regime SL/TP: range_pct`** or **`regime SL/TP: v_rank`** (from stored run params).
  - Inputs: capital, commission-per-side (USD per contract per fill side), qty, slippage ticks, optional **stop loss (ticks)** and **take profit (ticks)** for run-wide mechanical exits; **Scale template SL/TP by regime (entry bar)** checkbox plus **Regime mode** select (`range_pct` \| `v_rank`) that map to **`regime_exit_scale_enabled`** / **`regime_exit_scale_mode`** on **`POST /api/backtest/run`** when scaling is checked. When either run-wide SL or TP field has a numeric value, the checkbox is **disabled** and scaling is off (same mutual exclusion as the API). Unchecked scaling omits regime keys so merged **`config/backtest_defaults.json`** applies. Plus **execution policy** checkboxes (flip on opposite fire, honor SL intrabar, honor TP intrabar, flatten at end of window, **entry next bar open (integrity)**), optional **gap guard (ticks)** when next-bar mode is on (blank omits the key). Same-side stacking remains unsupported — UI copy explains skips when already long/short on that side; **`ignore_same_side_fire_when_open=false`** is rejected by server validation until pyramiding exists. Blank SL/TP inputs omit those keys from `POST /api/backtest/run` so the server keeps the merged broker defaults (`config/backtest_defaults.json` plus code fallbacks); numeric values override per key (same semantics as other optional broker fields). Execution-policy keys follow the same pattern: omitted keys keep merged defaults; present booleans override. On load, the client fills broker and execution fields from **`GET /api/backtest/defaults`** once the resolved **`apiBase`** is known (see §2.2 origin probe). When **`resolvedPath`** is empty or ends with **`config/backtest_defaults.json`** (standard repo file), **`GET /api/backtest/defaults`** is followed by **`GET /config/backtest_defaults.json`** (same origin, `cache: no-store`), and **`{ ...apiBroker, ...repoBroker }`** plus **`{ ...apiExecution, ...repoExecution }`** are applied — so sliders and execution toggles match the checked-in JSON even if the FastAPI process uses another cwd/env copy or an older read. When **`ORDERFLOW_BACKTEST_CONFIG`** resolves to a **different** filename, that overlay is skipped (server broker and execution are shown). When the GET fails, the client falls back to repo JSON only. With **`?source=api`**, repo JSON is also applied **optimistically** at boot (before `/sessions`). **`tick_size`** / **`point_value`** in **`POST`** use the final merged broker blob (or DOM where applicable).
