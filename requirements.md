...
- Pipeline: `pipeline/src/orderflow_pipeline/*` computes bars, events, fires, regime ranks (`v_rank`, `d_rank`), session VWAP, directional bias states, and DB writes.
  ...
  - CLI DB ingest (`cli.py` `_write_session_to_db`): a session **must** produce at least one canonical fire at **`1m`** (dense bars); at **`15m`** / **`1h`** it may produce **zero** fires for that session+timeframe (quiet regime-filtered window) — bars and related rows still write; **`fires`** for that slice stays empty.
  - DuckDB schema keys rows by `(bar_time, timeframe)` and keeps per-timeframe isolation for bars/events/fires/profile rows.
  - `bars` rows additionally carry `vwap`, PHAT features (`top_cvd`, `bottom_cvd`, `top_cvd_norm`, `bottom_cvd_norm`, `cvd_imbalance`, `top_body_volume_ratio`, `bottom_body_volume_ratio`, `upper_wick_liquidity`, `lower_wick_liquidity`, `high_before_low`, `rejection_side`, `rejection_strength`, `rejection_type`), and bias columns (`bias_state`, `parent_1h_bias`, `parent_15m_bias`).
...
- `/bars`, `/events`, `/fires` payloads include `vwap`, `barEndTime` (exclusive bar end, ISO-Z), PHAT fields (`topCvd`, `bottomCvd`, `topCvdNorm`, `bottomCvdNorm`, `cvdImbalance`, `topBodyVolumeRatio`, `bottomBodyVolumeRatio`, `upperWickLiquidity`, `lowerWickLiquidity`, `highBeforeLow`, `rejectionSide`, `rejectionStrength`, `rejectionType`), and bias fields (`biasState`, `biasH1`, `bias15m`) projected from persisted columns via `_attach_htf_bias`.
...
- **Chart view mode state:** `candleMode` (`standard` | `phat`) with PHAT availability inferred from loaded API bars; `phatBodyImbalanceThreshold` (default **0.30**) gates P/b body shading and `phatExhaustionRingLiquidityThreshold` (default **0.55**) controls exhaustion-ring fill when `rejectionType === 'exhaustion'` (see §7.1).
...
- The matrix renders a point-cloud overlay (`.matrix-point-layer`) with **one point per loaded bar in the active matrix range window** — the same **`from` / `to`** bounds as matrix occupancy and heatmap (`resolveOccupancyWindow()` in `src/ui/matrixRange.js`: Current RTH, Last hour, Last N sessions, All loaded, Custom). This is **not** tied to the chart viewport width. The **live forming** bar is included when its time falls in that window and it is not already represented in `replay.allBars`. In synthetic replay or when the occupancy window cannot be resolved, the overlay falls back to **chart-viewport-visible** bars (prior behavior).
...
- Opacity is relative to the **point-cloud bar list** (range window or viewport fallback): oldest point in that list trends to `POINT_MIN_OPACITY`, newest to `POINT_MAX_OPACITY`.
- **Volume → dot size (decoupled from chart viewport):** base radius scales with **`sqrt(volume)`** winsorized at **p5–p95** on sorted sqrt volumes over the **loaded timeframe bar set** (`replay.allBars` in API replay; rolling `state.bars` in synthetic). Percentile cutoffs are **`MATRIX_LADDER_LO_PCT` / `MATRIX_LADDER_HI_PCT`** in `src/analytics/matrixLadderConstants.js` (shared with the delta ladder). Mapping runs through `src/analytics/matrixVolumeRadiusNorm.js` into a band around **`POINT_RADIUS_PX`** (defaults **0.65×–1.35×**). Tail clamping keeps one-off spikes from dominating the ladder; spike-heavy sessions compress extremes vs widening to **p1–p99** (future tunable). The ladder recomputes when that loaded set changes (reload / TF switch), **not** when the chart pans within the same load — so panning does not rescale dots for cross-slice comparison on the regime surface.
- **Delta → dot fill (orthogonal to volume radius):** steady-state point **fill** encodes **signed bar `delta`** (aggressor imbalance): **green** when `delta > 0`, **red** when `delta < 0`. **Magnitude** interpolates **saturation and lightness in HSL** between fixed anchors per sign (`src/analytics/matrixDeltaColorNorm.js`), with **`|delta|`** winsorized over the same loaded bar universe using the **shared** ladder percentiles. **`delta === 0`** or non-finite **`delta`** uses a **neutral cool gray** (`hsl(210, 12%, 64%)` fill, **`hsl(210, 12%, 46%)` stroke**) so directionally flat flow stays visible on dark matrix chrome at minimum age opacity (stroke backstops the disk edge). Pulse/hover/selection behavior unchanged for steady fills aside from white stroke on hover/selection.
- **Chart candles vs matrix points (intentional divergence):** standard price candles are colored by **OHLC direction** (close vs open — §7.1). Matrix point **fills** follow **`delta`**, not candle direction. A **green candle with a red matrix point** (or the reverse) means **price vs flow disagreement**; that pairing is **expected** when surfacing aggressive imbalance against the bar’s print.
- **Hover / selection:** radii multiply the volume-derived base (**`MATRIX_POINT_HOVER_RADIUS_MULT` ~1.3**, **`MATRIX_POINT_SELECTED_RADIUS_MULT` ~1.5** in `matrix.js`) so interaction does not flatten relative volume ordering; hover/selection strokes remain **highlight white** over the delta-derived ring color.
- Default visual tokens are centralized in `src/render/matrix.js` constants (`POINT_MIN_OPACITY`, `POINT_MAX_OPACITY`, `POINT_RADIUS_PX`, hover/selected multipliers, jitter radius, stroke width) and in **`matrixDeltaColorNorm.js`** / **`matrixLadderConstants.js`** for delta/HSL tuning.
- **Directional pulse (candle-linked, not delta-linked):** when `close < open`, the matrix dot uses **`matrix-point--bear-flash`** (red core + halo pulse) while **hover-linked**, **selected**, or **live forming** bear (pulse pauses when the pointer hovers another bar’s link). When `close >= open`, **`matrix-point--bull-flash`** applies the same pulse structure in **chart-up green** (`#00c087` family). These pulses reflect **bar direction** for interaction parity with the chart, **not** delta sign — steady-state fill color remains delta-encoded when pulse is off. Pulse halo spread uses per-element **`--pulse-ring-min` / `--pulse-ring-max`** (derived from the rendered dot radius) so halos scale with dot size instead of fixed pixel spreads alone.
- Morphology colorization is a hook only: point style resolution is routed through `resolvePointStyle(bar, opts)` and can later map `bar.morphologyClass` to colors without changing point-cloud data flow.
...
- **Layout / theme:** `#priceChart` CSS height is **465px** (tall main chart). The price canvas and `.price-wrap` use **`#000000`** background; OHLC + volume sub-band candles use **Webull-style** greens/reds (`CHART_CANDLE_UP` **`#00c087`**, **`CHART_CANDLE_DOWN`** **`#ff3b30`** from `src/config/constants.js`, also mirrored in CSS **`--up` / `--down`**). The delta lane canvas matches the same **black** + palette.
...
- When PHAT mode is active, an **`i`** control beside the Candle selector (**click**) opens a **centered modal** (`#phatLegendModalOverlay` / `src/ui/phatLegendModal.js`) over a dimmed backdrop — **not** the HTF bias ribbon hover popover. Close with the **×** control, **Escape**, or a **backdrop click**. While open, **`Escape`** must not also clear brush selection (`selection.js` gates on `isPhatLegendModalOpen()`). The dialog summarizes **neutral** (side-by-side translucent green/red swatches, **disagreement** copy) vs **P-shape** vs **b-shape**, **body width vs volume** (narrow/wide mini-candles + viewport-relative scaling copy), **wick-tip rejection** (**filled** / **open** circle + diameter row), and footer notes including matrix volume encoding (§6.1). Layout: `orderflow_dashboard.html` + `styles/dashboard.css` (wide panel, scrollable).
- **PHAT reference:** `demo_files/candle_prototype.html` is the authoritative base form (asymmetric wicks, body-width tiering, rejection rings), while body shading intentionally diverges to the threshold-gated model from `notes.txt`.
...
- **Optional PHAT research (not blocking UI):** `python scripts/phat_wick_liquidity_distribution.py` reads DuckDB `bars` read-only and prints wick-liquidity / wick-length histograms (plus CSV under `data/` by default) to study the empirical liquidity distribution (`docs/cursor-prompt-wick-liquidity-histogram.md`).
...
- At most one rejection ring when `rejectionStrength > 0` (`rejectionSide`, `rejectionStrength`, `rejectionType`); radius 2–5px from strength. **Ring fill:** `rejectionType === 'absorption'` → filled; `exhaustion` → filled only if wick liquidity on the rejection side is **`>= state.phatExhaustionRingLiquidityThreshold`** (default **0.55**), otherwise hollow (chart-background fill + colored stroke). When brush selection is active and a bar is dimmed (matrix cells / fire window / outside the brushed set), rejection ring **fill and stroke** use the same **×0.45** alpha factor as the PHAT body so markers stay visually tied to their candle.
- **PHAT hover classification tooltip:** in PHAT mode, hovering a candle body emits a `phatCandle` hit ([`src/ui/tooltip.js`](src/ui/tooltip.js)). **Visual hierarchy:** compact **header** (e.g. `P-SHAPE · UPPER HALF · Wide body`, neutral variants); **gestalt line** immediately below (quoted italic interpretation — plain-language read first); optional **⚠** when Option A **disagreement**; **measurements** as a **three-column label / number / note grid** (Vol. (view) third, imbalance vs gate **G** + band, delta + matrix color words); **rejection** on two lines when present — summary (`Upper wick · exhaustion · open ring`) plus sub-line for liquidity vs fill threshold and qualitative strength (no pixel diameter). Viewport-rescaling and pipeline tie-break copy stay in the **legend**, not the tooltip. **Footer:** “PHAT read is descriptive, not predictive”. Payload from [`src/render/priceChart.js`](src/render/priceChart.js) `_buildPhatHoverPayload`. Strength buckets: `weak` / `moderate` / `strong` from `rejectionStrength` thresholds above.
...
- **`col-left`:** Header, stream controls, **Price · Volume Profile** block (section head with **`#replayRow`** timeframe + candle controls, canvas, chart pan, legend), **Delta Distribution**, **Signals & glossary**.
