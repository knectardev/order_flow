...
function evolveSimState() {
  ...
  if (state.scenarioLockBars > 0 && state.scenarioLockCell) {
    state.scenarioLockBars--;
    state.sim.volState   = state.scenarioLockCell.volState;
    state.sim.depthState = state.scenarioLockCell.depthState;
    if (state.scenarioLockBars === 0) state.scenarioLockCell = null;
    return;
  }
...
function generateBar() {
  ...
  if (state.primeNextSweep && state.bars.length >= 10) {
  ...
  if (state.primedDisplacement > 0) {
    const driftAmount = state.primedDirection * 0.55;
